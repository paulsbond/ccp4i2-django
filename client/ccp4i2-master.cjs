...
async function startCCP4i2() {
  ...
  server.use((req, res, next) => {
    res.setHeader(
      ...
      `default-src 'self'; connect-src http://localhost:${NEXT_PORT} ; style-src 'self' https://cdn.jsdelivr.net 'unsafe-inline'; script-src 'self' https://cdn.jsdelivr.net 'unsafe-inline' 'unsafe-eval'`
    );
    next();
  });
