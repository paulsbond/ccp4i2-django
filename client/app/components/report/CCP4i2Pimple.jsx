...
class CCP4GraphPlot {
  constructor(graphDivName_in, showInput) {
    var $graphDivOuter = $(`#${graphDivName_in}`);
    var graphDivOuter = $graphDivOuter.get(0);
    ...
    var parentWidth = parseInt($graphDivOuter.width());
    var parentHeight = parseInt($graphDivOuter.height());
    var w = parentWidth + "px";
    var h = parentHeight - 30 + "px"; //FIXME Hardwire should be (approx) 2 X max(menu font,input font) + 10.

    let $menuContainer = $("<div>").css("width", $graphDivOuter.css("width"));
    ...
    $menuSelect.change(function () {
      $("#" + self.menuSelectName + " option:selected").each(function () {
        ...
        thisPlotDiv.setAttribute(
          "style",
          "width:" +
            thisPlotDiv.style.width +
            ";height:" +
            thisPlotDiv.style.height +
            ";display:block;"
        );
        $("#" + graphName).empty();
        ...
        self.plot();
        var selectedIndex = $("#" + self.menuSelectName).prop("selectedIndex");
      });
    });
  }
}
...
CCP4GraphPlot.prototype.loadXML = function (graphObject, root, tables) {
  ...
  var pimpleplot = document.getElementById(graphObject.graphDivName);
  var igraph = 0;
  ...
  for (var itab = 0; itab < tables.length; itab++) {
    ...
    var data = $.trim(
      getNodeText(tables[itab].getElementsByTagName("data")[0])
    ).split("\n");
    //Martin Noble changed this...not all tables from ccp4i2 include explicit separator
    var headers_sep = " ";
    if (
      tables[itab].getElementsByTagName("headers")[0].hasAttribute("separator")
    ) {
      headers_sep = tables[itab]
        .getElementsByTagName("headers")[0]
        .getAttribute("separator");
    }
    var headers = $.trim(
      tables[itab].getElementsByTagName("headers")[0].childNodes[0].nodeValue
    )
      .replace(/ +/g, " ")
      .split(headers_sep);
    var dataColRow = [];
    ...
    for (var iplot = 0; iplot < plots.length; iplot++) {
      ...
      if (plotNodes.length > 0) {
        plot_description = plotNodes[0].childNodes[0].nodeValue
          .trim()
          .replace("<", "&lt;")
          .replace(">", "&gt;")
          .replace(/\n/g, "<br>");
      ...
      for (var iplotline = 0; iplotline < plotlines.length; iplotline++) {
        ...
        try {
          label =
            plotline.getElementsByTagName("label")[0].childNodes[0].nodeValue;
        } catch (err) {
          label = headers[ycol];
        }
        ...
        try {
          ...
          if (averagebatches == "true" || averagebatches == "1") {
            ...
            averagebatchcol =
              parseInt(
                plotline.getElementsByTagName("averagebatchcol")[0]
        ...
        try {
          var showinlegend =
            plotline.getElementsByTagName("showinlegend")[0].childNodes[0]
              .nodeValue;
          if (
            showinlegend &&
            (showinlegend == "false" || showinlegend == "0")
          ) {
            theData = { data: d1 };
          } else {
        ...
        try {
          var linestyle =
            plotline.getElementsByTagName("linestyle")[0].childNodes[0]
              .nodeValue;
          if (linestyle == "--") {
            //dash
        ...
        try {
          ...
          if (plotline.getElementsByTagName("symbol").length > 0) {
            symbol =
              plotline.getElementsByTagName("symbol")[0].childNodes[0]
                .nodeValue;
            if (symbol == "o") {
              symbol = "circle";
            } else if (symbol == "d") {
          ...
          if (plotline.getElementsByTagName("fillcolour").length > 0) {
            fillcolour =
              plotline.getElementsByTagName("fillcolour")[0].childNodes[0]
                .nodeValue;
            if (fillcolour == "false") {
              if (symbol === "none") {
        ...
        try {
          var symbolsize =
            parseInt(
              plotline.getElementsByTagName("symbolsize")[0].childNodes[0]
                .nodeValue
            ) /
              2 +
            1;
      ...
      try {
        ...
        for (var ilstrip = 0; ilstrip < linestrips.length; ilstrip++) {
          var dataPoints = linestrips[ilstrip].childNodes[0].nodeValue
            .trim()
            .split(/\s+/);
          var p = [];
          for (var ip = 0; ip < dataPoints.length / 2; ip++) {
      ...
      try {
        ...
        for (var ipoly = 0; ipoly < polygons.length; ipoly++) {
          var dataPoints = polygons[ipoly].childNodes[0].nodeValue
            .trim()
            .split(/\s+/);
          var p = [];
          for (var ip = 0; ip < dataPoints.length / 2; ip++) {
      ...
      try {
        fixaspectratio =
          plot.getElementsByTagName("fixaspectratio")[0].childNodes[0]
            .nodeValue;
      ...
      for (var ihist = 0; ihist < hists.length; ihist++) {
        ...
        try {
          barplacement =
            plot.getElementsByTagName("barplacement")[0].childNodes[0]
              .nodeValue;
        } catch (err) {
          barplacement = null;
        }
      ...
      try {
        var customTicksLabelText =
          plot.getElementsByTagName("customXLabels")[0].childNodes[0].nodeValue;
      ...
      try {
        var customTicksLabelText =
          plot.getElementsByTagName("customYLabels")[0].childNodes[0].nodeValue;
        if (customTicksLabelText.split(",").length > 0) {
          var splitLabelTicks = customTicksLabelText.split(",");
          customYLabels = [];
          for (var iTick = 0; iTick < splitLabelTicks.length; iTick++) {
      ...
      selectItem.innerHTML = plot_title
        .replace("<", "&lt;")
        .replace(">", "&gt;");
      ...
      thisPlotDiv.addEventListener("plotHover", function (e) {
        var pimpleMain = document.getElementById(this.graphDivName_in);
        ...
        pimpleMain.hoverevt.initCustomEvent("graphHover", false, false, {
          x: e.detail.x,
          y: e.detail.y,
          plot: this.iplot,
          table: this.itab,
      ...
      thisPlotDiv.addEventListener("plotClick", function (e) {
        ...
        pimpleMain.clickevt.initCustomEvent("graphClick", false, false, {
          x: e.detail.x,
          y: e.detail.y,
          plot: this.iplot,
          table: this.itab,
  ...
  if (graphObject.graphs.length > 0) {
    ...
    $("#" + graphObject.menuSelectName + " option:eq(0)").prop(
      "selected",
      true
    );
    var graphName = $("#" + graphObject.menuSelectName)[0].value.replace(
      "_selectitem",
      ""
    );
  ...
  for (var ishape = 0; ishape < shapes.length; ishape++) {
    ...
    if ("linestrip" in shape) {
      ...
      if (canvas.getContext) {
        ...
        var canvasCoord = thePlot.p2c({
          x: linestrip["data"][0][0],
          y: linestrip["data"][0][1],
        });
        if (yaxisOverride != 1) {
          var canvasCoordy = this.p2cNY(
            thePlot,
            { y: linestrip["data"][0][1] },
            yaxisOverride
          );
...
CCP4GraphPlot.prototype.plot = async function () {
  ...
  if (!this.haveLaunchButton) {
    ...
    var launchButton = document.createElement("object");
    launchButton.setAttribute(
      "style",
      "float:right; position:relative; width:100%;"
    );
    launchButton.setAttribute("class", "qt_launch");
    launchButton.setAttribute("type", "x-ccp4-widget/CLauncherButton");
    var launchParam1 = document.createElement("param");
    launchParam1.setAttribute("name", "label");
    launchParam1.setAttribute("value", "");
    launchButton.appendChild(launchParam1);
    var launchParam2 = document.createElement("param");
    launchParam2.setAttribute("name", "exe");
    launchParam2.setAttribute("value", "loggraph");
    launchButton.appendChild(launchParam2);
    var launchParam3 = document.createElement("param");
    launchParam3.setAttribute("name", "icon");
    launchParam3.setAttribute("value", "zoom.svg");
    launchButton.appendChild(launchParam3);
    var launchParam4 = document.createElement("param");
    launchParam4.setAttribute("name", "tooltip");
    launchParam4.setAttribute("value", "Open graph viewer");
    launchButton.appendChild(launchParam4);
    this.launchDataIDParam = document.createElement("param");
    this.launchDataIDParam.setAttribute("name", "ccp4_data_id");
    this.launchDataIDParam.setAttribute("value", this.dataID);
    launchButton.appendChild(this.launchDataIDParam);
    this.launchDataIDCurrentIndex = document.createElement("param");
    this.launchDataIDCurrentIndex.setAttribute(
      "name",
      "ccp4_data_current_index"
    );
    this.launchDataIDCurrentIndex.setAttribute("value", -1);
    launchButton.appendChild(this.launchDataIDCurrentIndex);
    this.launchJobIdParam = document.createElement("param");
    this.launchJobIdParam.setAttribute("name", "jobId");
    this.launchJobIdParam.setAttribute("value", "");
    launchButton.appendChild(this.launchJobIdParam);
    //console.log("Adding button for " + this.dataID);
  }
  var current_idx = this.graphs.indexOf(this.currentGraph);
  this.launchDataIDCurrentIndex.setAttribute("value", current_idx);
  ...
  if (typeof xbreak !== "undefined" && xbreak.length > 0) {
    ...
    var pcwidth;
    var lastchar = outer.style.width.substring(
      outer.style.width.length - 1,
      outer.style.width.length
    );
    if (lastchar === "%") {
    ...
    for (var ibr = 0; ibr < xax.length; ibr++) {
      var bDName = new Array(N + 1)
        .join((Math.random().toString(36) + "00000000000000000").slice(2, 18))
        .slice(0, N);
      var b = document.createElement("div");
      b.setAttribute(
        "style",
        "width:" + pcwidth + ";height:" + outer.style.height + ";float:left"
      );
    ...
    if (xlabel) {
      ...
      if (!isNaN(h)) {
        ...
        divTop.setAttribute(
          "style",
          "width:100%;height:" + (hTop - 1) + "px;float:top;"
        );
        for (var ib = 0; ib < bs.length; ib++) {
          bs[ib].setAttribute(
            "style",
            "width:" + pcwidth + ";height:" + hTop + "px;float:left;"
          );
        }
      }
    ...
    for (var ib = 0; ib < bs.length; ib++) {
      ...
      if (ib < bs.length - 1) {
        ...
        if (options["xscale"] === "oneoversqrt") {
          xaxes[0]["tickFormatter"] = function (x) {
            var y = 1.0 / Math.sqrt(x);
            if (isFinite(y)) {
              return y.toFixed(2);
            } else {
              return "Inf";
        ...
        if (options["xscale"] === "log") {
          xaxes[0]["transform"] = function (v) {
            return v == 0 ? null : Math.log(v);
          };
          xaxes[0]["inverseTransform"] = function (v) {
            return Math.exp(v);
          };
          xaxes[0]["ticks"] = 4;
        }
        if (options["yscale"] === "log") {
          yax[ib][0]["transform"] = function (v) {
            return v == 0 ? null : Math.log(v);
          };
          yax[ib][0]["inverseTransform"] = function (v) {
            return Math.exp(v);
          };
          yax[ib][0]["ticks"] = 4;
          yax[ib][1]["transform"] = function (v) {
            return v == 0 ? null : Math.log(v);
          };
          yax[ib][1]["inverseTransform"] = function (v) {
            return Math.exp(v);
          };
          yax[ib][1]["ticks"] = 4;
        }
        ...
        if (usePlotly && false) {
          ...
          import("jquery.flot").then((result) => {
        ...
        if (options["xscale"] === "oneoversqrt") {
          xaxes[0]["tickFormatter"] = function (x) {
            var y = 1.0 / Math.sqrt(x);
            if (isFinite(y)) {
              return (1.0 / Math.sqrt(x)).toFixed(2);
            } else {
              return "Inf";
            }
          };
        ...
        if (usePlotly) {
          ...
          thePlot2 = await import("jquery.flot").then((result) =>
      ...
      $("#" + bDName).bind("plotclick", function (event, pos, item) {
        if (item) {
          ...
          if (options["xscale"] === "oneoversqrt") {
            ...
            theRealOuterDiv.clickevtone.initCustomEvent(
              "plotClick",
              false,
              false,
              { x: datax, y: item["datapoint"][1] }
            );
          } else {
            theRealOuterDiv.clickevtone.initCustomEvent(
              "plotClick",
      ...
      $("#" + bDName).bind("plothover", function (event, pos, item) {
        if (item) {
          ...
          if (options["xscale"] === "oneoversqrt") {
            ...
            theRealOuterDiv.hoverevtone.initCustomEvent(
              "plotHover",
              false,
              false,
              { x: datax, y: item["datapoint"][1] }
    ...
    if ("ryrange" in options) {
      ...
      if (yr["max"] < yr["min"]) {
        ...
        var ymax = options["ryrange"]["max"];
        var ymin = options["ryrange"]["min"];
        yr["max"] = ymin;
        yr["min"] = ymax;
      }
      yaxes.push(yr);
    ...
    if (options["fixaspectratio"] === true) {
      ...
      outer.setAttribute(
        "style",
        "width:" + theSize + "px;height:" + theSize + "px;float:top;"
      );
    ...
    if (xlabel) {
      ...
      if (!isNaN(h)) {
        ...
        divTop.setAttribute(
          "style",
          "width:100%;height:" + hTop + "px;float:top;"
        );
      }
      divBot.innerHTML = xlabel.trim().replace(/\s+/, "&nbsp;");
    } else {
    ...
    if (options["yscale"] === "log") {
      yaxes[0]["transform"] = function (v) {
        return v == 0 ? null : Math.log(v);
      };
      yaxes[0]["inverseTransform"] = function (v) {
        return Math.exp(v);
      };
      yaxes[0]["ticks"] = 4;
    }
    ...
    if (options["xrange"]["max"] < options["xrange"]["min"]) {
      ...
      var xmax = options["xrange"]["max"];
      var xmin = options["xrange"]["min"];
      xaxes[0]["max"] = xmin;
      xaxes[0]["min"] = xmax;
    }
    if (options["xintegral"] === true) {
    ...
    $("#" + graphName).bind("plotclick", function (event, pos, item) {
      if (item) {
        ...
        if (options["xscale"] === "oneoversqrt") {
          ...
          theRealOuterDiv.clickevttwo.initCustomEvent(
            "plotClick",
            false,
            false,
            { x: datax, y: item["datapoint"][1] }
    ...
    $("#" + graphName).bind("plothover", function (event, pos, item) {
      if (item) {
        ...
        if (options["xscale"] === "oneoversqrt") {
          ...
          //console.log(datax);
          theRealOuterDiv.hoverevttwo.initCustomEvent(
            "plotHover",
            false,
            false,
            { x: datax, y: item["datapoint"][1] }
          );
          self.dataInfoDiv.innerHTML =
            "" + datax.toFixed(4) + ", " + item["datapoint"][1].toFixed(4);
        } else {
          theRealOuterDiv.hoverevttwo.initCustomEvent(
            "plotHover",
            false,
          ...
          self.dataInfoDiv.innerHTML =
            "" +
            item["datapoint"][0].toFixed(4) +
            ", " +
            item["datapoint"][1].toFixed(4);
        }
...
CCP4GraphPlot.prototype.loadFromUrl = function (url) {
  ...
  var extendedURL =
    "/jobFile?jobId=" + $("div[data-jobid]").data("jobid") + "&filePath=" + url;
