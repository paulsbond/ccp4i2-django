...
class FakeDb(object):
    class FakeSignal:
        def emit(self, *arg, **kwarg):
            logger.info(f"Ive been asked to emit {arg}, {kwarg}")
    ...
    def __getattribute__(self, __name):
        logger.debug(f"Fakedb being interrogated for {__name}")
    ...
    def getFileByJobContext(
        ...
        while len(searchJobsList) > 0:
            ...
            logger.info(f"Looking for context in {contextJob.jobnumber}")
            ...
            logger.debug(f"Using filterDict {filterDict}")
    ...
    def getTaskNameLookup(self, projectId=None, jobId=None, extras=False):
        ...
        try:
            logger.warning(
                f"In unimplemented toutine getTaskNameLookup {projectId}, {jobId}, {extras}"
            )
        except Exception as err:
            logger.warning(f"Err in unimplemented routine getTaskNameLookup {err}")
    ...
    def deleteFilesOnJobNumberAndParamName(self, projectId=None, jobNumberParamList=[]):
        try:
            ...
            logger.info(f"Err in deleteFilesOnJobNumberAndParamName {err}")
            traceback.print_stack()
        return None
    ...
    def getJobInfo(
        ...
        try:
            ...
            logger.info(f"Response from getJobInfo is {result}")
            traceback.print_stack()
            return result
        except Exception as err:
            logger.info(f"Err in getJobInfo {err} {mode} {returnType}")
            traceback.print_stack()
        return None
    ...
    def gleanJobFiles(
        ...
        try:
                    ...
                    if isinstance(
                        ...
                        if len(inputOutputFile.getFullPath()) > 0:
                            try:
                                ...
                                print(
                                    "ObjectDoesNotExist issue gleaning {} - file not found".format(
                                        jobParamName
                                    )
                                )
                                continue
            ...
            logger.info(f"Err in gleanFiles {err}")
            traceback.print_stack()
        return None

    def findInputOutputs(self, ofContainer, role, inputOutputsFound=[]):
        if isinstance(ofContainer, str):
            logger.info(f"Cant findInputOutputs of string {ofContainer}")
...
class FakeProjectsManager(object):
    ...
    def __getattribute__(self, __name):
        logger.info(f"FakeProjectsManager being interrogated for {__name}")
        return super().__getattribute__(__name)
    ...
    def getProjectDirectory(self, projectName=None, testAlias=True, projectId=None):
        print(
            "*****In FakeGetProjectDirectory {}, {}, {}".format(
                projectName, testAlias, projectId
            )
        )
        if projectId is not None:
            # Baffling edge case
...
def UsingFakePM(func):
    def wrapper(*args, **kwargs):
        ...
        try:
            ...
            logging.error(f"Encountered issue while in FakePM decorator {err}")
            traceback.print_exc()
        finally:
...
def cloneJob(jobId=None):
    ...
    logger.info(f"newJobDir {newJobDir}, newJobId {newJobId}")
    from core import CCP4TaskManager
    ...
    logger.info(f"theJobPlugin {theJobPlugin}")
...
def saveParamsForJob(theJobPlugin, theJob, mode="JOB_INPUT", excludeUnset=True):
    # logger.info(f'into saveParams for {theJob.jobnumber} excludeUnset: {excludeUnset}')
    # sys.stdout.flush()
    fileName = theJobPlugin.makeFileName(mode)
    # logger.info(f'saveParams in {fileName}')
    # sys.stdout.flush()
    ...
    # logger.info(f'out of saveParams for {theJob.jobnumber} excludeUnset: {excludeUnset}')
    # sys.stdout.flush()
